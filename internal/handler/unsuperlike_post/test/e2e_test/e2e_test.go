package unsuperlike_post_e2e_test
...
func TestUnsuperlikePost(t *testing.T) {
	log.Info().Msg("Starting UnsuperlikePost E2E test")
	...
	e2e_test_arrange.AddSuperlikePost(t, superlikePost)
	userUnsuperlikedPostEvent := &event.UserUnsuperlikedPostEvent{
		Username: superlikePost.Username,
		PostId:   superlikePost.PostId,
	}
